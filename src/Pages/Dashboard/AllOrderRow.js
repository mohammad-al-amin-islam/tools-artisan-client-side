...
const AllOrderRow = ({ order, index, refetch, setUnpaidItem }) => {
    ...
    return (
        <tr>
            ...
            <td>{paid !== true ? <label onClick={() => setUnpaidItem(order)} htmlFor="my-modal-3" className="btn btn-xs modal-button">Cancel</label> : <span>Paid order cannot delete</span>}</td>
        </tr>
    );
};
